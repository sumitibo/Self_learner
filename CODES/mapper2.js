...
const priceMapper = ({ data }) => {
  ...
  function getPriceRange({ price, price_zone, price_group, tier_quantity }) {
    for (const key in priceRangeMapper) {
      const rangeChecker =
  ...
  for (let i = 0; i < data.length; i += 1) {
    ...
    mainData[key] = `${price}`;
    getPriceRange({ price, price_zone, price_group, tier_quantity });
  }
